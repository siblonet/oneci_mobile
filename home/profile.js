...
export default function Profile({ navigation }) {
    ...
    const [fistnam, setFist] = useState();
    ...
    function Update_Pass() {
        ...
        if (passwordn === password && password != undefined && password !== passwordo) {
            ...
            axios.put(`${routx.tunal}/people/passwordupdate/${id}`, passwors).then((ddd) => {
    ...
    function Update_user() {
        ...
        const token = `${id}°${fistnam}°${name}°${call}°${autoryzed}`;

        axios.put(`${routx.tunal}/people/personupdate/${id}`, toUpdate)
            .then(() => {
                oneci.transaction((txn) => {
                    txn.executeSql(
                        'UPDATE oneci_instapay SET token=? WHERE id=?',
                        [token, "1"],
                        (tx, results) => {
                        ...
                        <View style={hilai.inpuCon}>
                            ...
                            <View style={{
                                ...
                                <TextInput
                                    ...
                                    onEndEditing={() => {
                                        setFoca(false);
                                        if (nom !== name | prenom !== fistnam | phone !== call) {
                                            setTouched(true);
                                        } else {
                                            setTouched(false);
                                        }
