...
export default function DashBoard({ navigation }) {
    ...
    function Bearerfetcher() {
        if (nni && capturedImage) {
            ...
            axios.get(`${routx.tunal}instapay/instapay`).then(answ => {
                if (answ.data.instapaytoken) {
                    requesttoBackendAtrubi('GET', `${routx.rnppUrl}/oneci/persons/${nni}?attributeNames=FIRST_NAME&attributeNames=LAST_NAME&attributeNames=BIRTH_DATE&attributeNames=GENDER`, answ.data.instapaytoken);
    ...
    const requesttoBackendAtrubi = async (method, endpoint, BEARER_TOKEN) => {
        ...
        if (!response.ok) {
            ...
            //console.log(responseData.LAST_NAME);
            setAccepted(true);
            setRejected(false);
            setIsLoaded(false);
        }
        ...
        <>
            {unlock ?
                ...
                >
                    ...
                    <View style={{
                        width: (capturedImage || userData) ? "70%" : "60%",
                        height: (capturedImage || userData) ? "40%" : "45%",
                        borderRadius: (capturedImage || userData) ? 25 : 120,
                        overflow: (capturedImage || userData) ? "hidden" : "visible",
                        alignItems: "center",
                        justifyContent: "center"
                        ...
                        {accepted && !capturedImage &&
                            <>
                                ...
                                <TouchableOpacity style={
                                    ...
                                    }>Prénom: {userData.FIRST_NAME}</Text>
                                    <View style={{ height: 20 }}>
                                    </View>
                                    <Text style={
                                        {
                                    ...
                                    }>Date de Naissance: {userData.BIRTH_DATE}</Text>
                                    <View style={{ height: 20 }}>
                                    </View>
                                    <Text style={
                                        {
                        ...
                        <Image
                            source={capturedImage ? { uri: `data:image/jpeg;base64,${capturedImage}` } : picts.faceid}
